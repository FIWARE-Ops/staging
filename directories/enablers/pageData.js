...
var modalData = [
    ...
    { 
        ...
        gitHubOrg: "https://github.com/telefonicaid/",
        docker: "https://hub.docker.com/r/telefonicaiot/fiware-sth-comet",
    ...
    { 
        ...
        gitHubOrg: "https://github.com/telefonicaid/",
        docker: "https://hub.docker.com/r/telefonicaiot/fiware-cygnus",
    ...
    { 
        ...
        gitHubOrg: "https://github.com/telefonicaid/",
        docker: "https://hub.docker.com/r/telefonicaiot/lightweightm2m-iotagent",
    ...
    { 
        ...
        gitHubOrg: "https://github.com/telefonicaid/",
        docker: "https://registry.hub.docker.com/r/telefonicaiot/iotagent-ul",
    ...
    { 
        ...
        gitHubOrg: "https://github.com/telefonicaid/",
        docker: "https://quay.io/repository/telefonicaiot/sigfox-iotagent",
    ...
    { 
        ...
        badge: "<img src='https://www.fiware.org/custom/brand-guide/img/badges/catalogue/02/svg/robotics.svg'/>",
        chapter: "interface", 
        content: "<p><img src='https://img.shields.io/github/license/Engineering-Research-and-Development/iotagent-aas.svg' alt='License' /> <img src='https://img.shields.io/github/release-date/Engineering-Research-and-Development/iotagent-aas.svg' alt='' /> <img src='https://img.shields.io/github/commits-since/Engineering-Research-and-Development/iotagent-aas/latest.svg' alt='' /></p> <p>IoT Agent accepting data from AAS devices.</p> <p>This IoT Agent is designed to be a bridge between the <a href='http://www.opcua.us/'>Administration Asset Shell</a> protocol and the <a href='https://swagger.lab.fiware.org/?url=https://raw.githubusercontent.com/Fiware/specifications/master/OpenAPI/ngsiv2/ngsiv2-openapi.json'>NGSI</a> interface. It follows the standard interaction model defined in the <a href='https://iotagent-node-lib.rtfd.io'>IoT Agent Framework Library</a>.</p> <h4 id='whatareiotagents'>What are IoT Agents?</h4> <p>The IoT Agent (IDAS) component is an implementation of the Backend Device Management GE, according to the FIWARE reference architecture. You are here because you need to connect objects to gather data or interact with them, typical IoT use case scenario . If so, you need to use one of the existing IoT Agents that are part of IDAS.</p> <p>You need this component if you plan to connect IoT devices/gateways to FIWARE-based ecosystems. IoT Agents translate IoT-specific protocols into the NGSI context information protocol, that is the FIWARE standard data exchange model. You do not need this component if your devices or gateways natively support the NGSI API.</p> <p>FIWARE-Ready IoT devices are able demonstrate the ability of the IoT solutions to seamless interoperate with FIWARE platforms. Any IoT device which is able to connect to the context broker (either directly or via an IoT Agent) is able to undergo a certification and testing process and can be added to the <a href='https://marketplace.fiware.org/'>FIWARE marketplace</a> and labelled as a <a href='http://marketplace.fiware.org/pages/how-to-devices'>FIWARE-Ready IoT Device</a>.</p> <h4 id='whyuseiotagents'>Why Use IoT Agents?</h4> <p>By using an IoT Agent , your devices will be represented in a FIWARE platform as NGSI entities in a Context Broker. This means that you can query or subscribe to changes of device parameters status by querying or subscribing to the corresponding NGSI entity attributes at the ContextBroker.</p> <p>Additionally, you will be able trigger commands to your actuation devices by updating specific command-related attributes in the associated NGSI entities representation at the Context Broker. This way, all hardware interactions with IoT devices can be handled by the Context Broker, using a homogeneous <a href='https://swagger.lab.fiware.org/?url=https://raw.githubusercontent.com/Fiware/specifications/master/OpenAPI/ngsiv2/ngsiv2-openapi.json'>NGSI</a> interface.</p>",  
        domain: "IoT Devices",  
        technology: "Angular",
        status: "Incubating",
    ...
    { 
        ...
        badge: "<img src='https://www.fiware.org/custom/brand-guide/img/badges/catalogue/04/svg/security.svg'/>",
        chapter: "context-data", 
        content: "<p><img src='https://img.shields.io/github/license/authzforce/server.svg' alt='License' /> <img src='https://img.shields.io/github/release-date/authzforce/server.svg' alt='' /> <img src='https://img.shields.io/github/commits-since/authzforce/server/latest.svg' alt='' /></p> <h4 id='whatisauthzforce'>What is Authzforce?</h4> <p>You get the reference implementation of the Authorization PDP Generic Enabler (formerly called Access Control GE). Indeed, as mandated by the GE specification, this implementation provides an API to get authorization decisions based on authorization policies, and authorization requests from PEPs. The API follows the REST architecture style, and complies with XACML v3.0. XACML (eXtensible Access Control Markup Language) is a OASIS standard for authorization policy format and evaluation logic, as well as for the authorization decision request/response format. The PDP (Policy Decision Point) and the PEP (Policy Enforcement Point) terms are defined in the XACML standard. This GEri plays the role of a PDP.</p> <p>To fulfill the XACML architecture, you may need a PEP (Policy Enforcement Point) to protect your application, which is not provided here. For REST APIs, we recommend you use the PEP Proxy by UPM available in the catalogue.</p> <h4 id='whyuseauthzforce'>Why use Authzforce?</h4> <p>Providing authorization for your application is a must for security reasons. However, it is always a complex part to implement, especially for non-security developers, because it involves advanced security concepts (Identity-based, RBAC, ABAC, etc.). Most developers embed the authorization logic within the application code, which makes it hard to maintain, evolve and integrate with external services providing extra authorization attributes. In this regard, the Authorization PDP helps you externalize the authorization logic and take advantage of flexible and standard-compliant Attribute-Based Access Control features. Combined with the Identity Management GE and the PEP proxy, this gives you a comprehensive access control solution for your application.</p> <p>The Authorization PDP specification defines a RESTful API of an Authorization Policy Decision Point (PDP) compliant with the OASIS XACML standard. More specifically, it defines RESTful interfaces for: Managing XACML-compliant authorization policies; Requesting authorization decisions based on those policies, in a XACML-compliant request-response format.</p>",  
        domain: "PDP",  
        technology: "XACML",
        status: "Full",
    ...
    { 
        ...
        badge: "<img src='https://www.fiware.org/custom/brand-guide/img/badges/catalogue/04/svg/api-management.svg'/>",
        chapter: "context-data", 
        content: "<p><img src='https://img.shields.io/github/license/FIWARE/kong-plugins-fiware.svg' alt='License' /> <img src='https://img.shields.io/github/release-date/FIWARE/kong-plugins-fiware.svg' alt='' /> <img src='https://img.shields.io/github/commits-since/FIWARE/kong-plugins-fiware/latest.svg' alt='' /></p> <h5 id='whatarefiwarekongplugins'>What are FIWARE Kong Plugins ?</h5> <p>Kong is a popular cloud-native API gateway acting as a reverse proxy ,that lets you manage, configure, and route requests to your APIs. The functionality of Kong can be extended through the use of plugins, allowing developers to introduce new security functionality within a  pre-existing API gateway framework</p> <p>The FIWARE Kong plugins offer targeted extensions to the existing framework simplifying connections to various components which are commonly used as part of the FIWARE Ecosystem (e.g. Keyrock, Keycloak)</p> <h5 id='whyusethefiwarekongplugins'>Why use the FIWARE Kong Plugins ?</h5> <p>When creating a FIWARE based system, it is necessary to introduce a security system to protect your data. FIWARE is agnostic to the security stack implemented in the system, but through using Kong it is possible to connect to the authentication and authorization mechanims of existing  IDMs, PDPs etc.</p>",  
        domain: "",  
        technology: "Keycloak,Keyrock,Kong",
        status: "Incubating",
        gitHub: "https://github.com/FIWARE/kong-plugins-fiware",
        gitHubOrg: "https://github.com/FIWARE/",
        docker: "",
        quay: "https://quay.io/repository/fiware/kong",
